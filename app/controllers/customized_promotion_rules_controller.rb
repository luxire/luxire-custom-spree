class CustomizedPromotionRulesController < CustomizedResourceController
  ...
  def initialize
   @path = 'spree/admin/promotion_rules'
  ...
  def validate_promotion_rule_type
    ...
    if !valid_promotion_rule_types.include?(params[:promotion_rule][:type])
      ...
      respond_to do |format|
        format.html { redirect_to spree.edit_admin_promotion_path(@promotion)}
        format.js   { render :layout => false }
        format.json { render plain: "Error validating promotion rules" }
      end
    end
  end
