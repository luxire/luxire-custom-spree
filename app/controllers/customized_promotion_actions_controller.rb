class CustomizedPromotionActionsController  < CustomizedResourceController
  before_action :load_promotion, only: [:create, :destroy]
  before_action :validate_promotion_action_type, only: :create

  def initialize
     @path = 'spree/admin/promotion_actions'
  ...
  def create
    ...
    respond_to do |format|
      ...
      format.json { render plain: "Promotion Action Created successfully" }
    end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to spree.edit_admin_promotion_path(@promotion)}
      format.js   { render :layout => false }
      format.json { render plain: "Promotion Action deleted successfully" }
  ...
  def validate_promotion_action_type
    ...
    if !valid_promotion_action_types.include?(params[:action_type])
      ...
      respond_to do |format|
        format.html { redirect_to spree.edit_admin_promotion_path(@promotion)}
        format.js   { render :layout => false }
        format.json { render plain: "Error validating promotion actions" }
      end
    end
  end
end
