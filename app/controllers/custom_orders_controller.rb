class CustomOrdersController < Spree::Api::BaseController
	...
	def recent_completed_orders
		...
		@orders = Spree::Order.where("spree_orders.completed_at IS NOT NULL and spree_orders.state = ?",'complete').order("completed_at desc").page(params[:page]).per(params[:per_page])
